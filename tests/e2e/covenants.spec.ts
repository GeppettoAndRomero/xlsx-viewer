...
test.describe('covenants', () => {
  ...
  test('links to related tools and its own engineering-notes article (#177)', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      expect(href, `related card ${i} is not a self-link`).not.toContain('/xlsx-viewer/');
    ...
    if (await blogLink.count()) {
      await expect(blogLink).toHaveAttribute('href', /^https:\/\/runlocally\.app\/blog\/xlsx-viewer\/$/);
    }
  });
  ...
  test('keeps no input data in the page URL after viewing (#7)', async ({ page }) => {
    await page.goto('/xlsx-viewer/');
  ...
  test('converts offline after the first online visit (#2)', async ({ page }) => {
    ...
    await page.reload();
    await waitReady(page);
    // Open a workbook online so the lazily loaded SheetJS chunk is cached too.
    await viewWorkbook(page);
    ...
    try {
      ...
      await waitReady(page);
      await viewWorkbook(page); // open the workbook with no network at all
    } finally {
